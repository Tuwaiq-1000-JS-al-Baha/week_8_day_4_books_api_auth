...
router.post("/login", async (req, res) => {
  try {
    ...
    const valid = await bcrypt.compare(password, user.password) //يعني قارن الباسورد بالباسورد المشفر لليوزر
    if (!valid) return res.status(400).send("password incorrect")
    const token = jwt.sign({ sub: user._id }, process.env.JWT_SECRET_KEY, { expiresIn: "90d" }) //مفتاح توكن ومدة الباسورد
    res.json(token)
  } catch (error) {
    res.status(500).send(error.message)
  }
